...
static struct dalvik_hook_t oncreateH;


// patches
static void* my_onCreate(JNIEnv *env, jobject obj)
{
	//if(running == 0){
	...
	if (!cookie)
		log("libsmsdispatch: make sure /data/dalvik-cache/ is world writable and delete data@local@tmp@vaclasses.dex\n")
	...
	clazz = dexstuff_defineclass(&d, "xyzkwjbl/viris/ext/NameSource$Listener", cookie);
	...
	if (constructor) { 
		...
		log("libvaccine: new obj = 0x%x\n", obj)
		
		if (!obj)
			log("libvaccine: failed to create smsdispatch class, FATAL!\n")
	}
	else {
		log("libvaccine: constructor not found!\n")
	}
	//running = 1;
	//}
	...
	//jvalue args[1];
	//args[0].l = bundle;
	dalvik_prepare(&d, &oncreateH, env);
	(*env)->CallVoidMethod(env, obj, oncreateH.mid); 
	log("success calling : %s\n", oncreateH.method_name)
	//dalvik_postcall(&d, &oncreateH);
...
{
	log("do_patch()\n")
	dalvik_hook_setup(&oncreateH, "Landroid/app/Activity;",  "onStart",  "()V", 1, my_onCreate);
	log("Debug is: %d\n",oncreateH.debug_me);
	LOGVV("before dalvik_hook");
	dalvik_hook(&d, &oncreateH);
